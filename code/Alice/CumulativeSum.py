...
from mpmath import ncdf as ncdf
#from scipy.stats import norm as norm

class CumulativeSums:

    @staticmethod
    def cumulative_sums_test(binary_data, mode=0, verbose=False):
        """
        ...
        start = int(floor(0.25 * floor(-float(length_of_binary_data) / float(abs_max)) + 1))
        end = int(floor(0.25 * floor(float(length_of_binary_data) / float(abs_max)) - 1))

        terms_one = []
        for k in range(start, end + 1):
            sub = ncdf((4 * k - 1) * abs_max / float(sqrt(length_of_binary_data)))
            terms_one.append(ncdf((4 * k + 1) * abs_max / float(sqrt(length_of_binary_data))) - sub)

        start = int(floor(0.25 * floor(-float(length_of_binary_data) / float(abs_max) - 3)))
        end = int(floor(0.25 * floor(float(length_of_binary_data) / float(abs_max)) - 1))

        terms_two = []
        for k in range(start, end + 1):
            sub = ncdf((4 * k + 1) * abs_max / float(sqrt(length_of_binary_data)))
            terms_two.append(ncdf((4 * k + 3) * abs_max / float(sqrt(length_of_binary_data))) - sub)
