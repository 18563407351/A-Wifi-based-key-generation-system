...
class FrequencyTest:
    ...
    def monobit_test(binary_data, verbose=False):
        ...
        ones to , that is, the number of ones and zeroes in a sequence should be about the same.
    ...
    @staticmethod
    def block_frequency(binary_data, block_size=128, verbose=False):
        """
        ...
        number_of_blocks = int(float(length_of_bit_string) / float(block_size))
 
        if number_of_blocks == 1:
            # For block size M=1, this test degenerates to test 1, the Frequency (Monobit) test.
            return FrequencyTest.monobit_test(binary_data[0:block_size], True)
        ...
        for counter in range(number_of_blocks):
            ...
            # Determine the proportion i of ones in each M-bit
            one_count = 0
            for bit in block_data:
                if bit == '1':
                    one_count += 1

            pi = float(one_count) / float(block_size)
        ...
        z = float(number_of_blocks) / 2
        a = float(result) / 2
        # Compute P-Value 
        p_value = gammainc(z=z, a=a, b=inf, regularized=True)
