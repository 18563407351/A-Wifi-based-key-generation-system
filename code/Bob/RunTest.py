...
class RunTest:
    ...
    @staticmethod
    def run_test(binary_data, verbose=False):
        """
        ...
        tau = 2 / float(sqrt(length_of_binary_data))

        one_count = binary_data.count('1')

        pi = float(one_count) / float(length_of_binary_data)
    ...
    @staticmethod
    def longest_one_block_test(binary_data, verbose=False):
        """
        ...
        number_of_blocks = int(float(length_of_binary_data) / float(m))
        block_start = 0
        block_end = m
        xObs = 0
