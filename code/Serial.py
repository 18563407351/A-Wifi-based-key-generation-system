...
class Serial:
    ...
    @staticmethod
    def serial_test(binary_data, verbose=False, pattern_length=16):
        """
        ...
        for i in range(3):
            ...
            sums[i] = (float(sums[i] * pow(2, pattern_length - i)) / length_of_binary_data) - length_of_binary_data
        ...
        z1 = float(pow(2, pattern_length - 1)) / 2
        a1 = float(nabla_01) / 2.0
        p_value_01 = gammainc(z=z1, a=a1, b=inf, regularized=True)
        z2 = float(pow(2, pattern_length - 2)) / 2
        a2 = float(nabla_02) / 2.0
        p_value_02 = gammainc(z=z2, a=a2, b=inf, regularized=True)
